...
It provides a comprehensive suite of over fifty risk and return metrics,
...
\node[box, fill=purple!10](plots)    at (12,2.5) {\texttt{.plots}};
\node[box, fill=purple!10](report)   at (12,1)   {\texttt{.report}};
% Arrows
\draw[arr] (prices) -- (pf);
\draw[arr] (pos)    -- (pf);
\draw[arr] (aum)    -- (pf);
...
jQuantStats computes rolling variants of the Sharpe ratio, Sortino ratio, and
volatility over a configurable look-back window $w$:
\begin{equation}
...
self-contained HTML file—embedding all Plotly charts as base64-encoded
JavaScript—that can be shared without external dependencies.  The underlying
template uses Jinja2 and is fully customisable.
...
\texttt{.prob\_sharpe\_ratio(series, sr*)} & Probabilistic Sharpe ratio \\
\texttt{.sortino(series, periods)} & Annualised Sortino ratio \\
\texttt{.adjusted\_sortino()} & Benchmark-adjusted Sortino \\
...
\texttt{.volatility(series, periods, annualize)} & Realised volatility \\
...
\texttt{.up\_capture\_ratio()} & Return during up-market periods \\
\texttt{.down\_capture\_ratio()} & Return during down-market periods \\
\texttt{.annual\_breakdown()} & Per-year statistics table \\
\texttt{.rolling\_sharpe(window)} & Rolling Sharpe series \\
\texttt{.rolling\_sortino(window)} & Rolling Sortino series \\
