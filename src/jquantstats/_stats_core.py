"""Core statistical metrics for financial returns data."""
...
# ── Core statistics mixin ────────────────────────────────────────────────────


class _CoreStatsMixin:
    """Mixin class providing core financial statistics methods.

    All fundamental return/risk metrics live here.  Rolling-window metrics
    are provided by :class:`~jquantstats._stats_rolling._RollingStatsMixin`.
    The concrete :class:`~jquantstats._stats.Stats` dataclass inherits from
    both and adds the ``data`` / ``all`` dataclass fields.
    ...
    def max_drawdown(self, series: pl.Series) -> float:
        ...
        """
        return _CoreStatsMixin.max_drawdown_single_series(series)
