...
def test_tail(data):
    ...
    pd.testing.assert_frame_equal(x.all(), data.all().tail())


def test_highwater_mark(data):
    """
    Tests that the highwater_mark() method correctly calculates the running maximum of prices.
    ...
    Verifies:
        ...
        2. The high-water mark is always greater than or equal to the corresponding price.
        3. The high-water mark is non-decreasing over time.
        4. The calculation works with both compounded=False and compounded=True.
    """
    # Test with compounded=False (default)
    hwm_simple = data.highwater_mark(compounded=False)
    prices_simple = data.prices(compounded=False)

    # Verify shape
    assert hwm_simple.shape == prices_simple.shape

    # Verify high-water mark is always >= price
    assert (hwm_simple >= prices_simple).all().all()

    # Verify high-water mark is non-decreasing
    for col in hwm_simple.columns:
        assert (hwm_simple[col].diff().fillna(0) >= 0).all()

    # Test with compounded=True
    hwm_compound = data.highwater_mark(compounded=True)
    prices_compound = data.prices(compounded=True)

    # Verify shape
    assert hwm_compound.shape == prices_compound.shape

    # Verify high-water mark is always >= price
    assert (hwm_compound >= prices_compound).all().all()

    # Verify high-water mark is non-decreasing
    for col in hwm_compound.columns:
        assert (hwm_compound[col].diff().fillna(0) >= 0).all()

    # Verify high-water mark equals cummax of prices
    pd.testing.assert_frame_equal(hwm_simple, prices_simple.cummax())
    pd.testing.assert_frame_equal(hwm_compound, prices_compound.cummax())
...
def test_copy(data):
    ...
    pd.testing.assert_frame_equal(data_copy.returns, data.returns)
    pd.testing.assert_series_equal(data_copy.benchmark, data.benchmark)
...
def test_resample(data):
    ...
    yearly_data = data.resample(resample="YE", compounded=False)
    ...
    monthly_data = data.resample(resample="ME", compounded=True)
    ...
    # Verify that compounded resampling is different from simple resampling
    monthly_simple = data.resample(resample="ME", compounded=False)
    assert not monthly_simple.returns.equals(monthly_data.returns)
